...
export default function Format(props) {
    ...
    const {
        ...
        kai_mail, kojin_mail, mynum, school, sotsu, major, comming, birth,
        sex, jp_contact_post, jp_contact_address, jp_contact_phone, jp_contact_home,
        motherland_post, motherland_address, motherland_phone, motherland_home,
        kinkyu_kankei, kinkyu_num, kinkyu_name, selectedEm, val, uval
    ...
    //入力情報をsend
    const send = async () => {
        const sendQ = window.confirm('この内容で登録しますか？')
        if (sendQ) {
            // em table
            const inputEm = await request('http://localhost:3001/inputEm',
                {
                    em_num: in_em_num, em_date: comming, dp_id: s_dp_id, un_id: s_un_id, em_kan_fist: kan_first, em_kan_last: kan_last,
                    em_huri_fist: huri_first, em_huri_last: huri_last, em_en_first: en_first, em_en_last: en_last, em_birthday: birth,
                    em_sex: sex, em_kai_mail: kai_mail, em_kojin_mail: kojin_mail, em_mynum: mynum, em_school: school, em_sotsu: sotsu,
                    em_major: major,
                });
            if(inputEm){
                alert('登録しました。')
        ...
        // // jp_contact table
        // await request('http://localhost:3001/input_jp_contact',
        // {em_num : in_em_num, jp_contact_post : jp_contact_post, jp_contact_address : jp_contact_address, 
        //  jp_contact_phone : jp_contact_phone, jp_contact_home : jp_contact_home})

        // // motherland table
        // await request('http://localhost:3001/input_motherland',{
        //     em_num : in_em_num, motherland_post : motherland_post, motherland_address : motherland_address,
        //     motherland_phone : motherland_phone, motherland_home : motherland_home
        // })

        // // kinkyu table
        // await request('http://localhost:3001/input_kinkyu',{
        //     em_num : in_em_num, kinkyu_kankei : kinkyu_kankei, kinkyu_num : kinkyu_num,
        //     kinkyu_name : kinkyu_name
        // })
    }
    return (
        <>
            ...
            <center>
                <div className='signUp-container'>
                    <div className='item memberNum'>
                        ...
                        <div className='memberNum-2'><Input onChange={what_em_num} /></div>
                    </div>
                    ...
                    <div className='item memberDate'>
                        ...
                        <div className='memberDate-2'><InputDate onChange={in_comming} /></div>
                    </div>
                    <div className='item dp'>
                        <div className='dp-1'>所属</div>
                        {/* 部署 */}
                        <div className='dp-2'>
                            <SelectBox options={dplist} onChange={selectBusho} val={val}></SelectBox>
                        </div>
                        {/* ユニット */}
                        <div className='dp-3'>
                            <SelectBox options={unlist} TorF={TorF} toSentaku={toSentaku} onChange={selectUn} val={uval}></SelectBox>
                        </div>
                    </div>
                    ...
                    <div className='item name'>
                        ...
                        {/* 漢字の姓 */}
                        <div className='dr'><Input onChange={in_kan_fist}></Input></div>
                        {/* 漢字の名 */}
                        <div className='d'><Input onChange={in_kan_last}></Input></div>

                        <div className='huri dr'>フリガナ</div>
                        {/* フリガナの姓 */}
                        <div className='dr'><Input onChange={in_huri_fist}></Input></div>
                        {/* フリガナの名 */}
                        <div className='d'><Input onChange={in_huri_last}></Input></div>

                        <div className='en r'>英文(PASSPORT)</div>
                        {/* 英文の姓 */}
                        <div className='r'><Input onChange={in_en_fist}></Input></div>
                        {/* 英文の名 */}
                        <div><Input onChange={in_en_last}></Input></div>
                    </div>
                    <div className='item birth'>
                        <div className='birth-1'>生年月日（西暦）</div>
                        <div className='birth-2'><InputDate onChange={in_birth} /></div>
                    </div>
                    <div className='item email'>
                        ...
                        <div className='email-2'>
                            <div className='kaisha'>
                                ...
                                <div className='kaisha-2'><Input onChange={in_kai_mail} /></div>
                            </div>
                            <div className='kojin'>
                                <div className='kojin-1'>個人</div>
                                <div className='kojin-2'><Input onChange={in_kojin_mail} /></div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className='item mynum'>
                        ...
                        <div className='mynum-2'><Input onChange={in_mynum} /></div>
                    </div>
                ...
                <div className='buttons'>
                    ...
                    <span className='buttons-1'>
                        <button type="button" class="btn btn-danger" onClick={() => history.goBack()}>戻る</button>
                    </span>
                </div>
